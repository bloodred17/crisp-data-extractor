...
export const crispExtractor = async () => {
  ...
  try {
    ...
    await page.type('input[name="email"]', process.env.USERNAME || 'username', { delay: 40 });
    await delay(1000);
    await page.type('input[name="password"]', process.env.PASSWORD || 'password', { delay: 40 });
    
    const articles: Crisp[] = [];
    page.on('response', async (response: HTTPResponse) => {
      if (response?.request()?.url()?.includes(`https://app.crisp.chat/api/v1/website/${ORG_ID}/helpdesk/locale/en/articles/1`)) {
    ...
    const [button] = await page.$x('//button[contains(.,"Sign in to Dashboard")]') as [HandleFor<HTMLElement>];
    await button.click();
      ...
      const intercepted = interceptor(
        ...
        [
          (response: HTTPResponse) => 
            response?.request()?.url() === `https://app.crisp.chat/api/v1/website/${ORG_ID}/helpdesk/locale/en/article/${article?.article_id}`,
          (response: HTTPResponse) => response?.request()?.method() === 'GET',
          (response: HTTPResponse) => response?.status() === 200,
        ]
      );

      await page.goto(url + '/en/' + article?.article_id, { waitUntil: 'networkidle0' });
      const { data } = (await intercepted) as CrispResponse;

      if (data) {
        const newPage = await browser.newPage();
        try {
          await newPage.goto(data?.url as string, { waitUntil: 'networkidle0' });
    
          data.html = await newPage.evaluate(() => document.querySelector('[role="article"]')?.innerHTML);
